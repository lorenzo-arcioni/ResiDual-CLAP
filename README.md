...
The W-MSA output (denoted $\mathbf{A}$ in the equation above) is decomposed exactly over individual attention heads. Denoting by $W^O$ the row slice of the output projection for head $h$, with $W^O \in \mathbb{R}^{d_h \times D_\ell}$:
...
|---|---|---|---|
| **Head** | `WindowAttention` (module hook) | Raw head output $\mathbf{H}$ then projected to $\widehat{\mathbf{H}}$ | $184 \times [N, 24]$ pre-proj, $184 \times [N, D_\ell]$ post-proj |
