...
\mathbb{R}^{N_\text{clips} N_w^\ell M \times d_h}$ for all $H_{\mathrm{tot}} = 184$ 
...
% ------------------------------------------------------------------
\paragraph{Interpretability of the learned $\boldsymbol{\lambda}$ weights.}
% ------------------------------------------------------------------

The optimised weight vectors $\{\boldsymbol{\lambda}_{\ell,b,h}\}$ encode
the adaptation strategy inferred by the optimiser for each attention head.
Auditing these weights post-optimisation — inspecting which PCA components
are amplified, attenuated, or suppressed, and correlating these choices
with the geometric and semantic properties of Appendix~\ref{app:head_analysis}
— could clarify whether the optimiser consistently targets semantically
coherent directions and whether the learned patterns generalise across
datasets. Such analysis would also help distinguish genuine task-driven
adaptation from optimiser artefacts, and could inform more principled
initialisation strategies beyond Eq.~\eqref{eq:lambda_init}.
