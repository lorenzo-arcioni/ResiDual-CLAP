...
core of the system. I begin by describing the CLAP dual-encoder model and
its configuration (\S\ref{app:clap_overview}). I then present the HTS-AT
...
(\S\ref{app:htsat}). I then derive the per-head residual decomposition
...
Table~\ref{tab:clap_config}. Throughout the entire analysis pipeline,
audio clips are processed one at a time (batch size $B = 1$); all
tensor shapes reported in this appendix omit the batch dimension for
clarity, as it plays no role in the per-head decomposition.
...
\begin{figure*}[t]
    ...
    \caption{HTS-AT hierarchical architecture. The model consists of four
        ...
        tokens with embedding dimension $D_0 = 96$ (in the Figure $D = D_0$); 
        after three PatchMerging
...
summarised in Table~\ref{tab:architecture}. I index the residual stream as
...
full network, unlike in isotropic vision transformers examined in~\cite{residual2024}.
...
output (see \S\ref{app:residual} for the full decomposition):
\begin{align}
    \widetilde{\mathbf{A}}_{\ell,b}
    &= \bigl[\mathbf{H}_{\ell,b,1} \|\cdots\| \mathbf{H}_{\ell,b,H_\ell}\bigr]
       \,W^O_{\ell,b} + \mathbf{b}^O_{\ell,b}
    &&\in \mathbb{R}^{N_w^\ell \times M \times D_\ell}, \label{eq:wmsa_proj} \\
    \mathbf{A}_{\ell,b}
    &= \mathrm{flatten}_{(N_w^\ell,\, M)}\!\left(\widetilde{\mathbf{A}}_{\ell,b}\right)
    &&\in \mathbb{R}^{S_\ell^2 \times D_\ell},
    \label{eq:wmsa_out}
\end{align}
where $S_\ell^2 = N_w^\ell \cdot M$ recovers the full token grid after
window reassembly via \texttt{window\_reverse}.
I analyse the raw head outputs $\mathbf{H}_{\ell,b,h}$ directly,
...
W-MSA output $\mathbf{A}_{\ell,b}$ (Eq. \ref{eq:wmsa_out})
...
\begin{align}
    ...
    \;\in\; \mathbb{R}^{S_\ell^2 \times D_\ell},
\end{align}
where I define the \emph{per-head projected contribution}
\begin{equation}
...
$\widehat{\mathbf{H}}_{\ell,b,h} \in \mathbb{R}^{N_w^\ell \times M \times D_\ell}$ & Per-head projected contribution, post-$W^O_{\ell,b}$, pre-flatten; see Eq.~\eqref{eq:hhat} \\
\midrule
