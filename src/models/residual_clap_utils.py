...
class _SimpleAudioDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, wrapper, dataset, max_samples: int = 1000):
        ...
        for cls, indices in class_indices.items():
            ...
            perm = torch.randperm(len(indices_t))[:samples_per_class]
            for i in indices_t[perm]:
                audio_path, _, _ = dataset[i.item()]
...
def build_pca_loader(
    ...
    """
    pca_ds = _SimpleAudioDataset(wrapper, dataset, max_samples=max_samples)
    return DataLoader(
        pca_ds,
...
class KFoldEvaluator:
    ...
    Uso:
        evaluator = KFoldEvaluator(
            ...
            n_splits=5,
            pca_samples=300,
            test_size=100,   # campioni per fold di test (None = usa tutti)
        )
    ...
    def __init__(
        ...
        self.label_indices = [self.class_to_idx[l] for l in self.all_labels]
    ...
    def run(self, verbose: bool = True) -> Dict:
        ...
        labels_arr = np.array(self.label_indices)
        
        baseline_accs = []
        residual_accs = []
        
        for fold_idx, (train_idx, test_idx) in enumerate(skf.split(indices, labels_arr)):
            if verbose:
                print(f"\n{'='*60}")
